...
submission_dir = '../data/result/dev'
files = os.listdir(submission_dir)


def eval_task1():
    # define path for scores file
    outf = open(os.path.join('../data/', 'scores_task1.txt'), 'w')
    ...
    if 'task1.txt' in files:
        ...
        with open('../data/dev_task1.txt') as f:
            for line in f:
                task1_gold.append(int(line.strip().split('\t')[0]))
        # task 1 scores
...
def eval_task2():
    # define path for scores file
    outf = open(os.path.join('data/', 'scores_task2.txt'), 'w')
    ...
    if 'task2.txt' in files:
        ...
        with open('data/dev_task2.txt') as f:
            for line in f:
                scores = [int(k) for k in line.strip().split('\t')[0].split(',')]
